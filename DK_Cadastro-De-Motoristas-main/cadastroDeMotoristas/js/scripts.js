...
function carregarDados(dados) {
  ...
  tabelaMotoristas = $("#tabelaMotoristas").DataTable({
    ...
    columns: [
      ...
      {
        data: "ID",
        render: function (dado, type, row, meta) {
          return (
            '<button type="button" class= "btnEditar" value="' + dado + '"><img src="./assets/edit.svg" alt="ícone de lápis" width= "20px"></button>' +
            '<button type="button" class= "btnExcluir" value="' + dado + '"><img src="./assets/lixeira.svg" width= "20px"></button>'
          );
        },
      },
    ],
  ...
  // Adiciona o evento de clique para excluir uma linha
  $('#tabelaMotoristas').on('click', '.btnExcluir', function () {
    var ID = $(this).val();
    // Filtra a lista de motoristas mantendo apenas aqueles com ID diferente da ID clicada
    motoristas = motoristas.filter(function (item) {
